...
export function* publicationIntegrityChecker(): SagaGenerator<void> {
    ...
    debug(`${directoryFileCorrectArray.length} uuid directory found on disk and ${publicationDocumentIdArray.length} publication found in database`);
    ...
    debug(`seems to have ${directoryFileCorrectArray.length - filesInpublicationUUIDDirectoryNotCorrect.length} publications correct and identified in publication directory`);
    debug("--------");
}
